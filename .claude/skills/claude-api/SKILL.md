...
name: claude-api
description: |-
  Claude API / Anthropic SDK reference for models, pricing, streaming, tools, MCP, agents, caching, tokens, and migrations. Trigger before Claude/Anthropic tasks, LLM questions, or provider-unstated agent/MCP/RAG/generation/debugging work; never answer changing facts from memory. Skip when another provider is explicitly named or project code targets another provider.
